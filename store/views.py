...
def download_invoice(request, order_id):
    ...
    html_string = render_to_string(
        "store/pdf/dynamic_invoice.html", {"order": order, "settings": settings}
    )
...
def download_shipping_label(request, order_id):
    ...
    html_string = render_to_string(
        "store/pdf/dynamic_label.html", {"order": order, "settings": settings}
    )
...
def home(request):
    ...
    # Asal sales data se best-selling products nikalna
    # (aakhri 90 din, quantity ke hisab se)

    ninety_days_ago = timezone.now() - timezone.timedelta(days=90)

    best_seller_ids = (
        OrderItem.objects.filter(order__created_at__gte=ninety_days_ago)
        .values("product_id")
    ...
    best_sellers = list(Product.objects.filter(id__in=best_seller_ids, is_active=True))
    best_sellers.sort(key=lambda p: list(best_seller_ids).index(p.id))
    ...
    context = {
        ...
        "new_arrivals": Product.objects.filter(is_active=True, is_new=True).order_by(
            "-created_at"
        )[:8],
    }
...
def checkout(request):
    ...
    total = sum(item.subtotal for item in items)
        ...
        # ---- Server-side validation (fake/junk order rokne ke liye) ----
        ...
        with transaction.atomic():
            ...
            if out_of_stock_items:
                error_msg = "Kuch products out of stock hain: " + ", ".join(
                    out_of_stock_items
                )
...
Thank you for shopping with Maryam Crockery ❤️"""
        ...
        admin_whatsapp = (
            site.whatsapp_number if site and site.whatsapp_number else "923223489220"
        )
        request.session["order_whatsapp_link"] = (
            f"https://wa.me/{admin_whatsapp}?text={quote(wa_message)}"
        )
    ...
    return render(
        request,
        "store/checkout.html",
        {"items": items, "total": total, "site": SiteSettings.objects.first()},
    )
...
def get_cart_drawer(request):
    ...
    for item in items:
        image_url = (
            item.product.image.url if getattr(item.product, "image", None) else ""
        )
...
def wishlist(request):
    ...
    return render(
        request, "store/wishlist.html", {"wishlist": wishlist_obj, "items": items}
    )
...
def privacy_policy(request):
    return render(
        request, "store/privacy_policy.html", {"site": SiteSettings.objects.first()}
    )


def terms_conditions(request):
    return render(
        request, "store/terms_conditions.html", {"site": SiteSettings.objects.first()}
    )


def refund_policy(request):
    return render(
        request, "store/refund_policy.html", {"site": SiteSettings.objects.first()}
    )
...
def order_success(request, order_id):
    ...
    return render(
        ...
        {"order": order, "whatsapp_link": whatsapp_link},
    )
...
def category_products(request, slug):
    ...
    products = Product.objects.filter(category=category, is_active=True).select_related(
        "subcategory"
    )
